...
**用户隔离**：非管理员角色只能查看/管理自己创建的 Key；管理员（角色 `data_scope` 为 `all`）可查看和管理租户内所有 Key。

### `POST /api/keys`

创建 API Key（返回完整 key，仅此一次）。`createdBy` 自动设为当前用户。

### `GET /api/keys`

API Key 列表（脱敏）。非管理员只返回自己创建的 Key，管理员返回租户内全部。

Query 参数：`page`、`pageSize`、`status`（active/disabled）。

### `GET /api/keys/:id`

获取 API Key 详情。非管理员只能查看自己的 Key，否则返回 404。

### `DELETE /api/keys/:id`

删除 API Key。非管理员只能删除自己的 Key，否则返回 404。

### `PATCH /api/keys/:id/status`

启用/禁用。非管理员只能修改自己的 Key，否则返回 404。
...
模型配置列表。`apiKey` 字段脱敏返回（保留前4后4，中间 `****`）。

### `POST /api/model-configs`

创建模型配置。响应返回完整 `apiKey`（一次性回显）。

### `GET /api/model-configs/:id`

获取模型配置。`apiKey` 字段脱敏返回。

### `PUT /api/model-configs/:id`

更新模型配置。响应返回完整 `apiKey`（一次性回显）。
...
| Key 类型 | Header | 格式 | 说明 |
|---|---|---|---|
| Workflow Key | `X-Workflow-Key` | `wf_...` | workflow 发布时生成的 `invokeKey` |
| API Key | `X-API-Key` | `sk-...` | 平台 API Key（`POST /api/keys` 创建） |

| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/ai/workflows/invoke/:slugOrId` | 执行已发布 workflow（202 + executionId） |
| GET | `/api/ai/workflows/invoke/executions/:executionId` | 查询执行状态 |

**Header**：

| Header | 必填 | 说明 |
|---|---|---|
| `X-Workflow-Key` | 二选一 | workflow 级调用密钥 |
| `X-API-Key` | 二选一 | 平台 API Key |
